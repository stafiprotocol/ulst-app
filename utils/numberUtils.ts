...
export function stakeAmountToBn(amount: string | number): bigint {
	const precision = 9; // 10^9
	const intAmount = Number(amount) * Math.pow(10, precision);
	return BigInt(intAmount) * BigInt(Math.pow(10, precision));
...
export function toChainAmount(amount: string | number, decimals: number) {
	return toBN(amount).mul(toBN(Math.pow(10, decimals)));
}
